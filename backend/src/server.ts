...
app.get("/health", (c) => {
  ...
  return c.json({
    ...
    ui_status: "ok",
    message: engine.lastError ?? "singularity capture engine",
    version: "0.1.0",
...
app.post("/chat", async (c) => {
  try {
    ...
    // A completed turn becomes cross-session graph memory immediately.
    if (body.session_id?.trim()) {
      const ingested = ingestChatSession({
      ...
      if (!ingested) {
        console.warn("[memory] completed chat could not be written to the graph");
      }
    }
 ...
 *
 * The renderer records a clip as 16-bit PCM and posts the raw bytes here with
 * the sample rate in a query parameter. The backend holds the AssemblyAI key
 * and returns both the verbatim transcript and the tidied rewrite. Names of
 * apps and windows the user has recently been in are passed as key terms so
 * they are transcribed correctly.
 */
app.post("/chat/dictation", async (c) => {
  try {
    const sampleRate = Number(c.req.query("sample_rate") ?? 16000);
    const channels = Number(c.req.query("channels") ?? 1);
    if (!Number.isFinite(sampleRate) || sampleRate < 8000 || sampleRate > 48000) {
      return c.json({ error: "sample_rate must be between 8000 and 48000" }, 400);
    }
    if (channels !== 1 && channels !== 2) {
      return c.json({ error: "channels must be 1 or 2" }, 400);
    }

    const pcm = Buffer.from(await c.req.arrayBuffer());
    if (pcm.byteLength === 0) {
      return c.json({ error: "audio body is required" }, 400);
    }

    const keyterms = getActivitySummary(
    ...
    const result = await transcribeDictation({
      ...
      channels,
      sttPrompt:
        "A person speaking to their personal AI assistant about their work, apps, meetings, and notes.",
      keyterms,
    });
    ...
    return c.json({
      ...
      rewrite_error: result.rewriteError,
    ...
    return c.json(
      { error: err instanceof Error ? err.message : "dictation failed" },
      500
    );
  }
});
...
app.post("/memory", async (c) => {
  try {
    ...
    if (!id) {
      return c.json({ error: "memory graph write failed" }, 503);
    }
...
export function startServer(): void {
  ...
  serve({ fetch: app.fetch, hostname: API_HOST, port: API_PORT }, () => {
    console.log(`[server] singularity api http://${API_HOST}:${API_PORT}`);
  });
}
