...
const API_PORT = Number(process.env.SINGULARITY_PORT ?? 3030);
...
function getBackendEntry(): { command: string; args: string[]; cwd: string } {
  ...
  return {
    ...
    // directory. Runtime data is explicitly routed to ~/.singularity below.
    cwd: app.getPath("userData"),
  };
}

function getEnvForBackend(): NodeJS.ProcessEnv {
  const dataDir = path.join(os.homedir(), ".singularity");
  ...
  return {
    ...process.env,
    ...dotenv,
    SINGULARITY_DATA_DIR: dataDir,
    SINGULARITY_PORT: String(API_PORT),
  };
}
...
export async function startBackend(
  ...
  backendUtilityProcess = utilityProcess.fork(entry, [], {
    ...
    stdio: "pipe",
    serviceName: "Singularity Capture Engine",
  });
