...
export interface AppConfig {
  ...
  stt_engine: string;
  dictation: { enabled: boolean; provider: string; max_seconds: number };
  data_dir: string;
}
...
export interface DictationResponse {
  /** Tidied transcript (filler words removed); falls back to verbatim. */
  text: string;
  verbatim: string;
  confidence: number;
  duration_ms: number;
...
export class BackendOfflineError extends Error {
  constructor(message = "The Singularity recorder is not running") {
    super(message);
...
export const api = {
  ...
  /**
   * Transcribe a recorded clip. Binary bodies go straight to the backend
   * rather than through the JSON IPC proxy.
   */
  dictate: async (pcm: ArrayBuffer, sampleRate: number, channels = 1) => {
    const base = await getBaseUrl();
    const res = await fetch(
      `${base}/chat/dictation?sample_rate=${sampleRate}&channels=${channels}`,
      { method: "POST", headers: { "Content-Type": "audio/pcm" }, body: pcm }
    );
    const payload = (await res.json().catch(() => ({}))) as Partial<DictationResponse> & {
      error?: string;
    };
    if (!res.ok) {
      throw new Error(payload.error ?? `Dictation failed (${res.status})`);
    }
    return payload as DictationResponse;
  },
